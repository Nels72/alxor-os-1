...
1. **Base de connaissance compagnies** — état 2026-06-11 :
   ...
   - Testé : chaîne complète OK jusqu'à l'appel Claude — **bloqué sur la clé API Anthropic** (credential n8n « Anthropic API » `Dopd4Kc6zgDhWooQ`, header `x-api-key`, valeur placeholder à remplacer). Une fois la clé posée : relancer le webhook sur le record AXA `rec0zItJNXQSPX4mB` (« Mon Auto ») pour valider end-to-end.
